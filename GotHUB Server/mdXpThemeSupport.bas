...
Public Function HasTheme() As Boolean
    Static lHasTheme    As Long
    ...
    If lHasTheme = 0 Then
        ...
        Else
            ...
            DllGetVersion uVer
            lHasTheme = IIf(uVer.dwMajor >= 6, -1, 1)
        End If
    End If
    HasTheme = (lHasTheme < 0)
    On Error GoTo 0
End Function
...
Public Function FixThemeSupport(oControls As Object) As Boolean
    ...
    If HasTheme() Then
        For Each oCtl In oControls
            ...
            ElseIf TypeOf oCtl Is VB.CommandButton Or _
...
Private Sub pvDrawButton( _
    ...
    If Len(oButton.Caption) > 0 Then
        ...
        If oPict Is Nothing Then
            Debug.Print rcClient.Bottom, rcText.Bottom
