...
Function EnumWnd(ByVal hwnd As Long, ByVal lParam As Long) As Long
     ...
     Dim IsVisible As Single, IsZoom As Single, hwndMenu As Long
     
     l_CapLen = GetWindowTextLength(hwnd)
     GetWindowText hwnd, s_Caption, l_CapLen + 1
     GetClassName hwnd, ClassName, 50
     IsVisible = IsWindowVisible(hwnd)
     IsZoom = GetWindowLong(hwnd, GWL_EXSTYLE)
     hwndMenu = GetParent(hwnd)

     Call Insert(frmMonitor.lvEnumList, hwnd, ClassName, s_Caption)
     
     EnumWnd = 1
End Function

Private Sub Insert(ByVal lv As ListView, ByVal hwnd As Long, ByVal TxClass As String, ByVal WndCaption As String)
     Dim i As Long
     Dim l_icon As Long
     Dim cnt As Long
     Dim HasNoOwner As Long, WindowStyle As Long
